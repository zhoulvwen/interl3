...
RUN echo 'zhou' | vncpasswd -f > $HOME/.vnc/passwd
...
RUN echo 'whoami ' >>/zhou.sh

RUN echo 'cd ' >>/zhou.sh

RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/zhou.sh

RUN echo 'cd /noVNC-1.2.0' >>/zhou.sh

RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/zhou.sh

RUN chmod 755 /zhou.sh
